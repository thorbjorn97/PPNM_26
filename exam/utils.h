...
// Boolean PASS/FAIL with optional exit reason
inline std::string format_status(bool pass, const std::string& exit_reason = "") {
    // Inject bold green [PASS] or bold red [FAIL]
    std::string status_str = pass ? "\033[1;32m[PASS]\033[0m" : "\033[1;31m[FAIL]\033[0m";
    
    // Append the reason if one is provided
    if (!exit_reason.empty()) {
        status_str += " (" + exit_reason + ")";
    }
